...
在上次的计算中出现的加Hubbard-U和不加Hubbard-U给出不同磁性基态的问题，需要验证Hubbard-U项对磁性基态的影响，因此需要测试不同U情况下的磁性基态；除此之外，不同赝势的选取可能对结果有影响(芯电子的数目不同)，因此需要测试其他赝势对磁性基态的影响，总结的工作目标如下：
 - [ ] 在原胞中测试不同Hubbard-U给出的磁性基态。
 - [ ] 在扭转21°(扭转的最小原胞)中测试不同的Hubbard-U给出的磁性基态。
 - [ ] 更换赝势(V_pv $\Rightarrow$ V_sv)在原胞中计算磁性基态。
 - [ ] 更换交换/关联形式(GGA $\Rightarrow$ LDA)在原胞中计算磁性基态。

```flow
st=>start:      开始（第i个粒子）
op1=>operation: 随机选择N个粒子中的S个（不含i）
op2=>operation: l
st->op1->op2
```
